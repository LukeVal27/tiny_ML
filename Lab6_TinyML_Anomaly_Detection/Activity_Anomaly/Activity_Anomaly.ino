...
// Threshold = 95th percentile of normal-training reconstruction error (TODO 4).
const float kReconstructionErrorThreshold = 0.5;
...
void loop() {
  ...
  if (IMU.accelerationAvailable()) {
    ...
    // The BMI270 reports acceleration in g, but the autoencoder was trained on
    // the mHealth dataset whose accelerometer channels are in m/s^2 (~9.8 at
    // rest). Convert so the board's inputs match the training distribution.
    x *= 9.80665f;
    y *= 9.80665f;
    z *= 9.80665f;
